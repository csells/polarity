...
(async()=>{const hashes=new Set(),preview=[];let sampleRate;
for(let region=0;region<6;region++){
 ...
 sampleRate=g.m._mgbawasm_sample_rate();const samples=[];const seconds=region===0?34:3,frames=Math.ceil(seconds*59.7275);
 ...
 if(region===0){let tail=0,n=0;for(let i=data.length-sampleRate*4;i<data.length;i+=2){tail+=data.readInt16LE(i)**2;n++;}assert.ok(Math.sqrt(tail/n)>300,'Music failed after the 32-second DMA loop');}
 ...
 hashes.add(crypto.createHash('sha256').update(data.subarray(0,sampleRate*4)).digest('hex'));preview.push(data.subarray(0,sampleRate*4*2));console.log(`[${region+1}/6] PCM audio RMS ${rms.toFixed(0)}, peak ${peak}, ${seconds}s including ${region===0?'loop boundary':'regional arrangement'}`);
}
assert.equal(hashes.size,6);const pcm=Buffer.concat(preview),h=Buffer.alloc(44);h.write('RIFF');h.writeUInt32LE(36+pcm.length,4);h.write('WAVEfmt ',8);h.writeUInt32LE(16,16);h.writeUInt16LE(1,20);h.writeUInt16LE(2,22);h.writeUInt32LE(sampleRate,24);h.writeUInt32LE(sampleRate*4,28);h.writeUInt16LE(4,32);h.writeUInt16LE(16,34);h.write('data',36);h.writeUInt32LE(pcm.length,40);fs.writeFileSync('artifacts/gba-soundtrack.wav',Buffer.concat([h,pcm]));console.log('PASS: six distinct native PCM arrangements, no clipping, sustained playback across DMA loop.');
