...
for(let region=0;region<6;region++){
 const save=new Uint8Array(32768);save.fill(255);save.fill(0,0,32);save[0]=65;save[1]=1;save[2]=region;save.fill(3,3,9);save[31]=37+region+18;
 const g=await boot(save);g.tap(8);g.tap(2);g.run(0,3);g.capture('gba-location-'+region);assert.equal(g.read('room_id'),region*3);
 const samples=[];const seconds=region===0?18:3,frames=Math.ceil(seconds*59.7275);
 ...
 if(region===0){let tail=0,n=0;for(let i=data.length-16384*4;i<data.length;i+=2){tail+=data.readInt16LE(i)**2;n++;}assert.ok(Math.sqrt(tail/n)>300,'Music failed after the 16-second DMA loop');}
