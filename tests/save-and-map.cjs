...
const s={};for(const l of fs.readFileSync('build/polarity.noi','utf8').split('\n')){const m=l.match(/^DEF _(player|game_mode|room_id|deaths|saved_checkpoint|camera|oldkeys|tick|city) 0x([0-9A-F]+)/);if(m)s[m[1]]=parseInt(m[2],16)}
s.saved_checkpoint=s.city+6;
...
// Walk through the real cyan socket and checkpoint, then verify both persist.
await p.evaluate(s=>{polarity.write(s.player,112*16&255);polarity.write(s.player+1,112*16>>8);polarity.write(s.player+2,2000&255);polarity.write(s.player+3,2000>>8);},s);
await p.keyboard.down('ArrowRight');await p.waitForFunction(a=>polarity.read(a)===20,s.saved_checkpoint);await p.keyboard.up('ArrowRight');await p.waitForTimeout(1100);assert.equal(await p.evaluate(a=>polarity.read(a),s.city+12),1,'signal socket persisted');
...
const x=await p.evaluate(a=>polarity.read(a)+256*polarity.read(a+1),s.player);assert.equal(x,160*16,'reload resumes at flag');assert.equal(await p.evaluate(a=>polarity.read(a),s.player+23),1,'reload restores signal bridge');
