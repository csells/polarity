...
unsigned int hash(State *s){
 unsigned int h=2166136261u;int v[]={s->x/24,s->y/24,s->vx/8,s->vy/8,s->phase,s->charge,s->coyote>0,s->buffer>0,s->dash,s->prev,s->lock,s->clock/8,s->defeated,s->boost};
 for(int i=0;i<14;i++)h=(h^(unsigned int)v[i])*16777619u;return h?h:1;
}
...
int main(int argc,char **argv){
 ...
 for(int r=first;r<last;r++){
  memset(seen,0,sizeof(seen));load_room(r);init_state(&nodes[0].s,r);nodes[0].parent=-1;nodes[0].cost=0;nodes[0].score=0;hn=0;push(0);int end=1,found=-1,best=0;
  while(hn&&end<MAX-20&&found<0){int n=pop();
   for(int a=0;a<(int)sizeof(actions);a++){
    ...
    int cost=nodes[n].cost+6,remain=616-s.x/16;
